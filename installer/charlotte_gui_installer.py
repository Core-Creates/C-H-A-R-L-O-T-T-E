...
def open_ghidra_installer():
    subprocess.Popen(
        [
            sys.executable,
            os.path.join(os.path.dirname(__file__), "ghidra", "ghidra_installer.py"),
        ]
    )
...
def open_binary_ninja_installer():
    subprocess.Popen(
        [
            sys.executable,
            os.path.join(
                os.path.dirname(__file__), "binary_ninja", "binary_ninja_installer.py"
            ),
        ]
    )
...
def show_re_window(parent):
    ...
    Label(win, text="Reverse Engineering Tools", font=("Arial", 14, "bold")).pack(
        pady=10
    )
    Button(
        win, text="Install Ghidra (Headless)", width=30, command=open_ghidra_installer
    ).pack(pady=5)
    Button(
        win,
        text="Install Binary Ninja (Free)",
        width=30,
        command=open_binary_ninja_installer,
    ).pack(pady=5)
...
def show_recon_window(parent):
    ...
    Button(
        win, text="Install JDK 21+ (Temurin)", width=30, command=open_jdk_download
    ).pack(pady=5)
    Button(win, text="Back", width=30, command=win.destroy).pack(pady=20)
...
def show_exploitation_window(parent):
    ...
    Button(
        win, text="Launch Metasploit Plugin", width=30, command=launch_metasploit_plugin
    ).pack(pady=5)
...
def main():
    ...
    Label(
        root, text="C-H-A-R-L-O-T-T-E GUI Installer", font=("Arial", 16, "bold")
    ).pack(pady=10)
    Label(root, text="Select a category:").pack(pady=5)

    Button(
        root,
        text="Reverse Engineering (re)",
        width=30,
        command=lambda: show_re_window(root),
    ).pack(pady=5)
    Button(
        root, text="Defensive", width=30, command=lambda: show_recon_window(root)
    ).pack(pady=5)
    Button(
        root, text="Vulnscan", width=30, command=lambda: show_vulnscan_window(root)
    ).pack(pady=5)
    Button(
        root, text="Offensive", width=30, command=lambda: show_exploitation_window(root)
    ).pack(pady=5)
