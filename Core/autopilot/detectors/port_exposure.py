...
class PortExposureDetector:
    def __init__(self, cfg: Dict[str, Any], limiter: TokenBucketLimiter, netmon: BandwidthMonitor, logger):
        self.cfg = cfg or {}
        ...
        if shutil.which("nmap"):
            ...
            try:
                ...
                for host, ports in opened.items():
                    ...
                    flagged = [p for p in ports if p in (21, 23, 445, 3389, 5900) or p < 1024]
                    if flagged:
                        shown = flagged[:6]
                        title = f"Exposed services on {host}: {shown}…" if len(flagged) > 6 else f"Exposed services on {host}: {flagged}"
                        issues.append(Issue(
                            title=title,
                            severity="high",
